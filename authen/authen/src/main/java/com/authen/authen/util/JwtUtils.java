package com.authen.authen.util;
...
public class JwtUtils {
    ...
    private static final long ACCESS_EXPIRATION_MS = 25 * 60 * 1000;
    ...
    private void addCookie(HttpServletResponse response, Token type, String token, long expiryMs) {
        String cookie = buildCookie(type.name(), token, expiryMs / 1000);
    ...
    public void removeToken(HttpServletResponse response, Token tokenType) {
        String cookie = buildCookie(tokenType.name(), "", 0);
        response.addHeader("Set-Cookie", cookie);
    }
